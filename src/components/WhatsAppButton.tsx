...
export default function WhatsAppButton() {
  const whatsappNumber = "918848332909";
  const message = encodeURIComponent("Hi! I'm interested in Dock Media School. I'd like to book a free consultation.");
  ...
  return (
    <a
      ...
      className="fixed bottom-6 right-6 z-45 flex items-center justify-center w-14 h-14 bg-[#25D366] text-white rounded-full shadow-lg hover:scale-110 hover:shadow-[#25D366]/40 active:scale-95 transition-all duration-300 group"
      aria-label="Contact on WhatsApp"
    >
      {/* Pulse rings */}
      <span className="absolute inset-0 rounded-full bg-[#25D366]/30 animate-ping opacity-75 group-hover:animate-none" />
      <span className="absolute inset-0 rounded-full bg-[#25D366]/20 animate-pulse-slow" />
